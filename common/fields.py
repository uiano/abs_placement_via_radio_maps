...
class VectorField(Field):
    def map_on_grid(self, fun):
        ...
        - `t_out`: tensor of shape `N x num_pts_z x num_pts_y x
          num_pts_x` where `t_out[n]` contains the values of the first
          entry returned by `fun` at all grid points. 
    ...
    def list_vals(self, exclude_disabled_pts=True):
        """ 
            ...
            matrix. """
        return self.grid.list_vals(
            self.t_values, exclude_disabled_pts=exclude_disabled_pts)
